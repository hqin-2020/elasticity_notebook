    ...
    while error > tol and count < max_iter:
        M1_μ_D0, M1_μ_D1, E_D1_coeff, Z2_tp1, G\
            = _inner_loop(D1, E_D1_coeff, E_D2_coeff, Λ22, Q, df, N,
                          ...
                          Z1Z1, M0_E_w, M0_E_ww, n_Y, n_Z, n_W, γ)
...
def return_coeffs(Lq):
    ...
    if Lq['ww'].any():
        ...
        coeffs[29] = Lq['ww'][0,8] 
        coeffs[30] = Lq['ww'][0,9]   
        coeffs[31] = Lq['ww'][0,10] 
        coeffs[32] = Lq['ww'][0,11] 
        coeffs[33] = Lq['ww'][0,12] 
        coeffs[34] = Lq['ww'][0,13]  
        coeffs[35] = Lq['ww'][0,14] 
        coeffs[36] = Lq['ww'][0,15]  
    return coeffs

# def display_formula(Lq, Var, method = '', deci = 4):
#     coeffs = np.around(return_coeffs(Lq),deci)
#     print(80*'-')
#     print(method)

#     display(Math(r'$ ' + Var + r' = \begin{{align*}}& {} +\begin{{bmatrix}}{}&{}\end{{bmatrix}} X_t^1 + \begin{{bmatrix}}{}&{}&{}&{}\end{{bmatrix}}W_t^1 \\'.format(coeffs[0],coeffs[1],coeffs[2],coeffs[3],coeffs[4],coeffs[5],coeffs[6])+\
#                  r'&+\begin{{bmatrix}}{}&{}\end{{bmatrix}}X_t^2'.format(coeffs[7],coeffs[8])+\
#                 r'+\begin{{bmatrix}}{}&{}&{}&{}\end{{bmatrix}}X_{{t}}^1\otimes X_{{t}}^1 \\'.format(coeffs[9],coeffs[10],coeffs[11],coeffs[12])+\
#                  r'&+\begin{{bmatrix}}{}&{}&{}&{}&{}&{}&{}&{}\end{{bmatrix}}X_{{t}}^1\otimes W_{{t+1}} \\'.format(coeffs[13],coeffs[14],coeffs[15],coeffs[16],coeffs[17],coeffs[18],coeffs[19],coeffs[20])+\
#                 r'&+\begin{{bmatrix}}{}&{}&{}&{}&{}&{}&{}&{}&{}&{}&{}&{}&{}&{}&{}&{}\end{{bmatrix}}W_{{t+1}}\otimes W_{{t+1}}\end{{align*}}$'.format(coeffs[21],coeffs[22],coeffs[23],coeffs[24],coeffs[25],coeffs[26],coeffs[27],coeffs[28],coeffs[29],coeffs[30],coeffs[31],coeffs[32],coeffs[33],coeffs[34],coeffs[35],coeffs[36])))

def display_formula(Lq, Var, method = '', deci = 4):
    coeffs = np.around(return_coeffs(Lq),deci)
    print(80*'-')
    print(method)
    c_part = r' = \begin{{aligned}}&{}'.format(coeffs[0])
    if coeffs[1] == 0:
        z1_part = r'{}'.format('')  
    elif coeffs[1] > 0:
        z1_part = r'+{}\ Z_t^1 '.format(coeffs[1])
    elif coeffs[1] < 0:
        z1_part = r'{}\ Z_t^1 '.format(coeffs[1])
    if coeffs[2] == 0:
        y1_part = r'{}'.format('')
    elif coeffs[2] > 0:
        y1_part = r'+{}\ Y_t^1 '.format(coeffs[2])
    elif coeffs[2] < 0:
        y1_part = r'{}\ Y_t^1 '.format(coeffs[2])
    w_part = r'{} '.format(' ') if coeffs[3] == coeffs[4] == coeffs[5] == coeffs[6] == 0 else r'+\begin{{bmatrix}}{}&{}&{}&{}\end{{bmatrix}}W_{{t+1}} '.format(coeffs[3],coeffs[4],coeffs[5],coeffs[6]) 
    if coeffs[7] == 0:
        z2_part = r'{} '.format(' ')
    elif coeffs[7] > 0:
        z2_part = r'+{}\ Z_t^2'.format(coeffs[7])
    elif coeffs[7] < 0:
        z2_part = r'{}\ Z_t^2'.format(coeffs[7])
    if coeffs[8] == 0:
        y2_part = r'{} '.format(' ')
    elif coeffs[8] > 0:
        y2_part = r'+{}\ Y_t^2'.format(coeffs[8])
    elif coeffs[8] < 0:
        y2_part = r'{}\ Y_t^2'.format(coeffs[8])
    if coeffs[9] == 0:
        z1z1_part = r'{} '.format(' ')
    elif coeffs[9] > 0:
        z1z1_part = r'+{} Z_t^1 Z_t^1'.format(coeffs[9])
    elif coeffs[9] < 0:
        z1z1_part = r'{} Z_t^1 Z_t^1'.format(coeffs[9]) 
    if coeffs[10] == 0:
        z1y1_part = r'{} '.format(' ')
    elif coeffs[10] > 0:
        z1y1_part = r'+{} Z_t^1 Y_t^1'.format(coeffs[10])
    elif coeffs[10] < 0:
        z1y1_part = r'{} Z_t^1 Y_t^1'.format(coeffs[10])
    if coeffs[11] == 0:
        y1z1_part = r'{} '.format(' ')
    elif coeffs[11] > 0:
        y1z1_part = r'+{} Y_t^1 Z_t^1'.format(coeffs[11])
    elif coeffs[11] < 0:
        y1z1_part = r'{} Y_t^1 Z_t^1'.format(coeffs[11])
    if coeffs[12]== 0:
        y1y1_part = r'{} '.format(' ')
    elif coeffs[12] > 0:
        y1y1_part = r'+{} Y_t^1 Y_t^1'.format(coeffs[12])
    elif coeffs[12] < 0:
        y1y1_part = r'{} Y_t^1 Y_t^1'.format(coeffs[12])
    z1w_part =  r'{} '.format(' ') if coeffs[13] == coeffs[14] == coeffs[15] == coeffs[16] ==0 else r'\\&+\begin{{bmatrix}}{}&{}&{}&{}\end{{bmatrix}}Z_{{t}}^1\otimes W_{{t+1}}'.format(coeffs[13],coeffs[14],coeffs[15],coeffs[16])
    y1w_part =  r'{} '.format(' ') if coeffs[17] == coeffs[18] == coeffs[19] == coeffs[20] ==0 else r'+\begin{{bmatrix}}{}&{}&{}&{}\end{{bmatrix}}Y_{{t}}^1\otimes W_{{t+1}}\\'.format(coeffs[17],coeffs[18],coeffs[19],coeffs[20])
    ww_part =  r'{} \end{{aligned}}'.format(' ') if coeffs[21] == coeffs[22]==coeffs[23]==coeffs[24]==coeffs[25]==coeffs[26]==coeffs[27]==coeffs[28]==coeffs[29]==coeffs[30]==coeffs[31]==coeffs[32]==coeffs[33]==coeffs[34]==coeffs[35]==coeffs[36] == 0 else r'&+\begin{{bmatrix}}{}&{}&{}&{}&{}&{}&{}&{}&{}&{}&{}&{}&{}&{}&{}&{}\end{{bmatrix}}W_{{t+1}}\otimes W_{{t+1}}\end{{align*}}$'.format(coeffs[21],coeffs[22],coeffs[23],coeffs[24],coeffs[25],coeffs[26],coeffs[27],coeffs[28],coeffs[29],coeffs[30],coeffs[31],coeffs[32],coeffs[33],coeffs[34],coeffs[35],coeffs[36])
    display(Math(r'$ ' + Var + c_part + z1_part+ y1_part + w_part + z2_part+ y2_part +z1z1_part+z1y1_part+y1z1_part+y1y1_part + z1w_part + y1w_part+ww_part))
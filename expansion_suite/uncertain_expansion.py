...
def uncertain_expansion(eq, ss, var_shape, args, scale_loc, c_adj_loc_list, tol = 1e-8, max_iter = 50):
    ...
    N_1_res = N_0_expansion(df, ss, var_shape, γ)
    util_sol = {'vmc1_t':no_N_res['vmc1_t'],\
                'rmc1_t':no_N_res['rmc1_t'],\
                'vmc2_t':N_1_res['vmc2_t'],\
                'rmc2_t':N_1_res['rmc2_t']}
    if len(c_adj_loc_list) == 0:
        c1_scale = no_N_res['X1_tp1'][scale_loc+2-1]
        c2_scale = N_1_res['X2_tp1'][scale_loc+2-1]
    else:
        c1_scale = no_N_res['X1_tp1'][scale_loc+2-1] + lq_sum([no_N_res['X1_tp1'][i+2-1]-no_N_res['X1_t'][i+2-1] for i in c_adj_loc_list])
        c2_scale = N_1_res['X2_tp1'][scale_loc+2-1]  + lq_sum([N_1_res['X2_tp1'][i+2-1] -N_1_res['X2_t'][i+2-1]  for i in c_adj_loc_list])
    log_N = form_log_N(γ, next_period(util_sol['vmc1_t'], no_N_res['Z1_tp1']), next_period(util_sol['vmc2_t'],N_1_res['Z1_tp1'],N_1_res['Z2_tp1']),c1_scale, c2_scale)
    ...
    while error > tol and i < max_iter:
        ...
        Z1_tp1, Z2_tp1, ψ_tilde_x, ψ_tilde_w, ψ_tilde_q, ψ_x2, ψ_tilde_xx, ψ_tilde_xw, ψ_tilde_xq, ψ_tilde_ww, ψ_tilde_wq, ψ_tilde_qq,\
             = update_state_evolution(no_N_res, N_cm, var_shape)

        ## Step 2: Update Jump Variables
        # First order
        Y1_t, X1_t, X1_tp1, schur_first, D, C_mix = solve_jump_first(df, util_sol, Z1_tp1, N_cm, var_shape)
        # Second order
        Y2_t, X2_t, X2_tp1, schur_second, D2, D_tilde, G = solve_jump_second(df, util_sol, Z1_tp1, Z2_tp1, X1_t, X1_tp1, N_cm, var_shape)

        ## Step 3: Update continuation value and change of measure
        if len(c_adj_loc_list) == 0:
            c1_scale = X1_tp1[scale_loc-1]
            c2_scale = X2_tp1[scale_loc-1]
        else:
            c1_scale = X1_tp1[scale_loc-1] + lq_sum([X1_tp1[i-1]-X1_t[i-1] for i in c_adj_loc_list])
            c2_scale = X2_tp1[scale_loc-1] + lq_sum([X2_tp1[i-1]-X2_t[i-1] for i in c_adj_loc_list])
        util_sol = solve_utility(γ, β, ρ, no_N_res['Z1_tp1'], no_N_res['Z2_tp1'], var_shape, ss[scale_loc+2-1], c1_scale, c2_scale)
    ...
    return ModelSolution({
                        ...
                        'no_N_res':no_N_res, 'N_1_res':N_1_res,
...
def solve_jump_constant_second(df_p, schur, var_shape, Z1_tp1, X1_t, X1_tp1, change_of_measure):
...
def solve_utility(γ, β, ρ, Z1_tp1, Z2_tp1, var_shape, c_scale0, c_scale1, c_scale2, tol = 1e-10):
    ...
    util_sol = {'vmc1_t':vmc1_t,'vmc2_t':vmc2_t,'vmc1_tp1':vmc1_tp1,'vmc2_tp1':vmc2_tp1,'rmc1_t':rmc1_t,'rmc2_t':rmc2_t,\
...
class ModelSolution(dict):
    ...
    def approximate_fun(self, fun, args=()):
        ...
        if self.second_order:
            temp1 = combine_second_order_terms(dfun, self.X1_t, self.X1_tp1)
...
def plug_in_df_first_order(df, var_shape, util_sol):
    ...
    for nonzero_loc in nonzero_vr_index:
        if nonzero_loc[1] == 0:
            ...
            for i in range(n_Z):   
                # vmc2_t to Z1_tZ1_t
                df_xtxt[[nonzero_loc[0]], (i+n_Y+1)*n_X-n_Z : (i+n_Y+1)*n_X] += new_zz_coefs[i]        ### to Each Z segment
            # vmc2_t to Z1_tq
    ...
    for nonzero_loc in nonzero_vr_index:
        if nonzero_loc[1] == 0:
            ...
            new_zz_coefs = np.split(new_zz_coef,n_Z,axis=1)
            for i in range(n_Z):   
                # vmc2_tp1 to Z1_tp1Z1_tp1
                df_xtp1xtp1[[nonzero_loc[0]], (i+n_Y+1)*n_X-n_Z : (i+n_Y+1)*n_X] += new_zz_coefs[i]     ### to Each Z segment
            # vmc2_tp1 to Z1_tp1q
    ...
    for nonzero_loc in nonzero_vr_index:
        ...
        for i in range(n_Z):
            # to Z1_tp1Z1_t
            df_xtxtp1[[nonzero_loc[0]], (i+n_Y+1)*n_X-n_Z : (i+n_Y+1)*n_X] += new_zz_coefs[i]              ### to Each Z segment
        # to Z1_tq
    ...
    for nonzero_loc in nonzero_vr_index:
        # to x1_tp1Z1_t
        for i in range(n_Z):
            df_xtxtp1[[nonzero_loc[0]], (i+n_Y)*n_X+nonzero_loc[1]] += nonzero_vr_df[nonzero_loc[0], nonzero_loc[1]] * util_sol['vmc1_t']['x'][0,i]              ### to Each YZ piece
        # to x1_tp1q
    ...
    for nonzero_loc in nonzero_vr_index:
        # to x1_tp1Z1_t
        for i in range(n_Z):
            df_xtxtp1[[nonzero_loc[0]], (i+n_Y)*n_X+nonzero_loc[1]] += nonzero_vr_df[nonzero_loc[0], nonzero_loc[1]] * util_sol['rmc1_t']['x'][0,i]               ### to Each YZ piece
        # to x1_tp1q
    ...
    for nonzero_loc in nonzero_vr_index:
        # to Z1_tp1x1_t
        df_xtxtp1[[nonzero_loc[0]], n_X*(nonzero_loc[1]+1)-n_Z:n_X*(nonzero_loc[1]+1)] += nonzero_vr_df[nonzero_loc[0], nonzero_loc[1]] * util_sol['vmc1_t']['x']  ### to Each YZ segment
        # to x1_tq
    ...
    for nonzero_loc in nonzero_vr_index:
        # to Z1_tp1x1_t
        df_xtxtp1[[nonzero_loc[0]], n_X*(nonzero_loc[1]+1)-n_Z:n_X*(nonzero_loc[1]+1)] += nonzero_vr_df[nonzero_loc[0], nonzero_loc[1]] * util_sol['rmc1_t']['x']  ### to Each YZ segment
        # to x1_tq
    ...
    for nonzero_loc in nonzero_vr_index:
        ...
        # to wtp1q
        df_wtp1q[[nonzero_loc[0]]] += nonzero_vr_df[nonzero_loc[0], nonzero_loc[1]] * util_sol['vmc1_t']['c'].item()
    ...
    for nonzero_loc in nonzero_vr_index:
        ...
        # to wtp1q
        df_wtp1q[[nonzero_loc[0]]] += nonzero_vr_df[nonzero_loc[0], nonzero_loc[1]] * util_sol['rmc1_t']['c'].item()
    ...
    for nonzero_loc in nonzero_vr_index:
        if nonzero_loc[1] == 0:
            ...
            # vmc1_tq to qq
            df_qq[[nonzero_loc[0]]]        += nonzero_vr_df[nonzero_loc[0],nonzero_loc[1]] * util_sol['vmc1_t']['c']*2

        elif nonzero_loc[1] == 1:
            # rmc1_tq to Z1_tq
            df_xtq[[nonzero_loc[0]],-n_Z:] += nonzero_vr_df[nonzero_loc[0],nonzero_loc[1]] * util_sol['rmc1_t']['x']
            # rmc1_tq to qq
            df_qq[[nonzero_loc[0]]]        += nonzero_vr_df[nonzero_loc[0],nonzero_loc[1]] * util_sol['rmc1_t']['c']*2

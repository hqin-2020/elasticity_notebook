...
from derivatives import compute_derivatives, VAR1_compute_derivatives


def recursive_expansion_fix(eq_cond, ss,  var_shape, γ,
...
def _inner_loop(D1, E_D1_coeff, E_D2_coeff, Λ22, Q, df, N,
    ...
    _, coeff_xtp1, coeff_xtp1xtp1 = np.split(G[0:1, :], np.cumsum([1, n_Z]),
                                             axis=1)
    B1 = N[0:1, :]@Z2_tp1['ww'] + coeff_xtp1xtp1@Z1Z1['ww'] + Z2_tp1['ww'][-1,:] + next_period(Y2_t[2],Z1_tp1,Z2_tp1)['ww']
    B2 = N[0:1, :]@Z2_tp1['xw'] + coeff_xtp1xtp1@Z1Z1['xw'] + Z2_tp1['xw'][-1,:] + next_period(Y2_t[2],Z1_tp1,Z2_tp1)['xw']
    B3 = N[0:1, :]@Z2_tp1['w'] + coeff_xtp1@Z1_tp1['w']+ coeff_xtp1xtp1@Z1Z1['w'] + Z2_tp1['w'][-1,:] + next_period(Y2_t[2],Z1_tp1,Z2_tp1)['w']
...
def VAR1_expansion(VAR, Z_ss, VAR_0, var_shape, second_order=True, args=()):
    
    n_Y, n_Z, n_W = var_shape
    W_0 = np.zeros(n_W)
    q_0 = 0.

    df = VAR1_compute_derivatives(f=lambda Z_t, W_tp1, q:
                                anp.atleast_1d(VAR(Z_t, W_tp1, q, *args)),
                                Z=[Z_ss, W_0, q_0],
                                second_order=second_order)
    VAR_0 = LinQuadVar({'c':VAR_0.reshape(n_Y,-1)},shape = (n_Y, n_Z, n_W)) 
    
    VAR_1 = LinQuadVar({'c':df['q'].reshape(n_Y,-1),
                         'x':df['zt'],
                         'w':df['wtp1']},shape = (n_Y, n_Z, n_W))
    if second_order:
        VAR_2 = LinQuadVar({'c':df['qq'].reshape(n_Y,-1),
                     'x':df['ztq']*2,
                     'w':df['wtp1q']*2,
                     'x2':df['zt'],
                     'xx':df['ztzt'],
                     'xw':df['ztwtp1']*2,
                     'ww':df['wtp1wtp1']},shape = (n_Y, n_Z, n_W))
        VAR_expansion =  VAR_0 + VAR_1 + VAR_2*0.5
        return VAR_expansion, VAR_0, VAR_1, VAR_2
    else:
        VAR_expansion =  VAR_0 + VAR_1
        return VAR_expansion, VAR_0, VAR_1
